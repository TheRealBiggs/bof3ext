...
Accessor<0x6BC620, void*> g_hWnd;
Accessor<0x6BC624, void*> g_hInstance;
ArrayAccessor<0x6C3F40, UnkStruct_4> stru_6C3F40;
ArrayAccessor<0x7CAE38, ActiveSprite> g_ActiveSprites;
ArrayAccessor<0x7CA9E0, float> g_TexCoordTable;
ArrayAccessor<0x903880, ENV> g_Env;
PointerAccessor<0x937F84, ENV> g_CurrentEnv;

Func<0x4FD030, void> ReadCfgFile;
Func<0x4FD110, void> sub_4FD110;
Func<0x5A9914, void, int, void(*__cdecl)(void)> SetGameMode;
Func<0x496B60, void> GameMode_First;
Func<0x4FC6A0, void> sub_4FC6A0;
Func<0x5A9907, void> sub_5A9907;
Func<0x5A7860, void, DISPENV*> PutDispEnv;
Func<0x5A7890, void, DRAWENV*> PutDrawEnv;
Func<0x454960, void> sub_454960;
Func<0x461F00, void> sub_461F00;
Func<0x587C70, void> sub_587C70;
Func<0x5A7960, void, void**, int> ClearOTagR;
Func<0x4FD230, void> sub_4FD230;
Func<0x5A6780, void> sub_5A6780;
Func<0x5A98A0, void> sub_5A98A0;
Func<0x4FD290, void> sub_4FD290;
...
bool DrawCapcomLogo() {
	...
	auto _x = (float)(wndSize.x * 0.7) / capcomLogo->width;
	auto _h = capcomLogo->height * _x;

	Vec2f size = { wndSize.x * 0.7, _h };
	Rectf rect = { wndSize.x * 0.15, (wndSize.y - _h) / 2, size.x, size.y };
...
void ProcessGpuPrim_ShadedQuad(GpuPrim_ShadedQuad* prim) {
	Renderer::Get().DrawQuad(
		{ prim->v1.x * *g_RenderScaleX, prim->v1.y * *g_RenderScaleY, prim->v1.z }, { prim->colour.r, prim->colour.g, prim->colour.b, 255 },
		{ prim->v2.x * *g_RenderScaleX, prim->v2.y * *g_RenderScaleY, prim->v2.z }, { prim->c2.r, prim->c2.g, prim->c2.b, 255 },
		{ prim->v3.x * *g_RenderScaleX, prim->v3.y * *g_RenderScaleY, prim->v3.z }, { prim->c3.r, prim->c3.g, prim->c3.b, 255 },
		{ prim->v4.x * *g_RenderScaleX, prim->v4.y * *g_RenderScaleY, prim->v4.z }, { prim->c4.r, prim->c4.g, prim->c4.b, 255 }
	);
}
...
void ProcessGpuPrim_SetDrawEnv(GpuPrim_SetDrawEnv* prim) {
	...
	g_DrawEnv->dfe = (int)prim->flags & 1;
	g_DrawEnv->dtd = (int)prim->flags & 2;
	g_DrawEnv->tpage = prim->texturePage;

	if (prim->rect == nullptr)
		return;

	g_DrawEnv->tw = *prim->rect;
}
...
void __cdecl DrawOTag(void** ordering_table) {
	...
	if (*ordering_table != (void*)-1) {
		...
		for (int i = 0; i < 128; ++i) {
			g_FontGlyphs[i].word10 = 0;
			g_ActiveSprites[i].word14 = 0;
		}
	}
}


Func<0x59EDF0, void> sub_59EDF0;
auto sub_59EDF0Hook() {

}

Func<0x59EB00, void, PSX_RECT*, uint8_t*> LoadImage;
Func<0x59EA70, void, PSX_RECT*, uint8_t*> LoadImage2;
...
FuncSTD<0x4FCB00, int, void*, void*, void*, int> _WinMain;
auto __stdcall _WinMainHook(auto hInstance, auto hPrevInstance, auto lpCmdLine, auto nShowCmd) {
	...
	*g_hInstance = hInstance;
	...
	std::memset(&stru_6C3F40, 0, sizeof(UnkStruct_4) * 32);
	std::memset(&g_CLUT, 0, sizeof(UnkStruct_7) * 512);
	std::memset(&g_FontGlyphs, 0, sizeof(FontGlyph) * 128);

	auto fg = &g_FontGlyphs;
	...
	*(uint32_t*)0x6C3A4C = 0;
	std::memset((void*)0x6BE9F8, 0, 0x1Cu);
	std::memset((void*)0x7CADE8, 0, 0x1Cu);
	std::memset(&g_ActiveSprites, 0, sizeof(ActiveSprite) * 128);
	std::memset((void*)0x7DED60, 0, 0xC0u);
	*(uint32_t*)0x6C3A48 = 0;
	*(uint32_t*)0x6C3A44 = 0;
	auto v17 = 0;
	auto v8 = &g_TexCoordTable;

	do {
		++v8;
		auto v9 = (double)v17++ * 0.00390625;
		*(v8 - 1) = v9 + 0.002000000094994903;
	...
	*g_RenderScaleY = renderScale;
	...
	auto wnd = glfwCreateWindow(wndSize.x, wndSize.y, "Breath of Fire III", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(wnd)) {
		if (isCapcomLogo) {
			...
			if (tick < someFloat) {
				renderer.Begin();
				{
					PutDispEnv(&g_CurrentEnv->disp);
					PutDrawEnv(&g_CurrentEnv->env);
					sub_454960();
					sub_461F00();
					DrawOTag(&g_CurrentEnv->ordering_table[7]);
				}
				renderer.End();
			...
			*(uint8_t*)0x905B89 ^= 1u;
			*(ENV**)0x937F84 = &g_Env[*(uint8_t*)0x905B89];
			ClearOTagR(g_CurrentEnv->ordering_table, 8);
			sub_4FD230();
			sub_5A6780();
...
Func<0x59EE50, void, void**> _DrawOTag;
